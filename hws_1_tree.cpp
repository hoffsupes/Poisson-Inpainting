...
Mat do_inpaint(Mat ss, Mat tt, Mat M)
{
...
Mat do_composite(Mat ss, Mat tt, Mat M)
{
...
Mat S,T,M;  
S = imread("1.jpg");         // Name of input image
T = imread("1_BG.jpg");
M = imread("final_1_mask.jpeg",0);         // Name of Mask


resize(T,T,S.size());

Mat Fin = do_composite(S,T,M);
imwrite("1_composite.jpg",Fin);     // Output image written
...
// @dassg@rpi.edu
// Code if not running or problematic can be searched for a better version in github.com/dassg or if the link fails, search "dassg" on github and this file should be in the most recent repository of the user
